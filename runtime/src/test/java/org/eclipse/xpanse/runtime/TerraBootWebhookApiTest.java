...
@CrossOrigin
@SpringBootTest(
        properties = {"spring.profiles.active=oauth,zitadel,zitadel-testbed,terra-boot,test,dev"})
@AutoConfigureMockMvc
public class TerraBootWebhookApiTest extends ApisTestCommon {
    ...
    void testTerraBootWebhookApis() throws Exception {
        testTerraBootWebhookApisThrowsException();
        testTerraBootWebhookApisWell();
    }

    @Test
    @WithJwt(file = "jwt_all_roles-no-policies.json")
    void testTerraBootWebhookApisThrowsException() throws Exception {
        // Setup
    ...
    void testTerraBootWebhookApisWell() throws Exception {
                ...
                new OclLoader()
                        .getOcl(
                                URI.create("file:src/test/resources/ocl_terraform_test.yml")
                                        .toURL());
        ocl.setName("TerraBootWebhookApiTest-1");
                ...
                new OclLoader()
                        .getOcl(
                                URI.create(
                                        ...
                                        .toURL());
        oclFromGit.setName("TerraBootWebhookApiTest-2");
    ...
    MockHttpServletResponse orderCallback(UUID orderId, TerraformResult deployResult)
        ...
        return mockMvc.perform(
                        post("/webhook/terra-boot/order/{orderId}", orderId)
                                ...
                                .header(
                                        ...
                                        "algorithm=HmacSHA256;headers=x-nonce-signature"
                                            + " x-timestamp-signature;signature=1f1c7793d863585803a4d36b5d6149e06bf8b3d8b0e6d9a4f87b9b9ba7fb9575"))
                .andReturn()
                .getResponse();
    }
}
