...
public class EnumColumnConstraintManage {
    ...
    public List<String> queryConstraintForH2TableEnumColumn(String tableName, String columnName) {
        String queryStatement =
                String.format(
                        "SELECT T2.CONSTRAINT_NAME FROM INFORMATION_SCHEMA.CONSTRAINT_COLUMN_USAGE"
                            + " T1, INFORMATION_SCHEMA.CHECK_CONSTRAINTS T2  WHERE"
                            + " T1.CONSTRAINT_NAME = T2.CONSTRAINT_NAME  AND TABLE_NAME = '%s' AND"
                            + " COLUMN_NAME = '%s'",
                        tableName, columnName);
     ...
     * @param enumValuesString the string of enum values.
     */
    public void addNewConstraintForH2TableEnumColumn(
            String tableName, String columnName, String constraintName, String enumValuesString) {
