...
public class ServiceDetailsViewManager {
    ...
    public List<DeployedService> listDeployedServices(Category category, Csp csp,
        ...
        serviceDeploymentEntities
                .forEach(this::updateServiceDeploymentStateAndServiceOrderByStoredTerraformResult);
    ...
    private void updateServiceDeploymentStateAndServiceOrderByStoredTerraformResult(
        ...
        updateServiceDeploymentState(serviceDeployment, serviceOrderEntity);
    }
    ...
    private void updateServiceDeploymentStateAndServiceOrder(
            ServiceDeploymentEntity serviceDeployment,
            ServiceOrderEntity serviceOrderEntity, ErrorType errorType,
    ...
    private void updateServiceDeploymentState(ServiceDeploymentEntity serviceDeployment,
            ServiceOrderEntity serviceOrderEntity) {
        if (Objects.nonNull(serviceOrderEntity)) {
            if (Duration.between(serviceOrderEntity.getStartedTime(),
                ...
                if (serviceOrderEntity.getHandler().equals(Handler.TERRAFORM_BOOT)) {
                    try {
                        ResponseEntity<TerraformResult> result = terraformBootTaskResultHelper
                                .retrieveTerraformResult(String
                                        .valueOf(serviceOrderEntity.getOrderId()));
    ...
    private void updateServiceDeploymentState(Boolean isSuccess,
            ServiceDeploymentEntity serviceDeployment) {
        if (isSuccess) {
            if (serviceDeployment.getServiceDeploymentState()
                    .equals(ServiceDeploymentState.DEPLOYING)) {
